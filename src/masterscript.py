...
def main(config_map, **kwargs):
    ...
    *kwargs*: all of the options passed into the script
    ...
    datasets_dir = input_settings['datasets_dir']

    if 'mappings' in input_settings:
        namespace_mappings = setup_mappings(datasets_dir, input_settings['mappings'], **kwargs)

    setup_dataset_files(datasets_dir, input_settings['datasets_to_download'], namespace_mappings, **kwargs)
...
def setup_dataset_files(datasets_dir, dataset_settings, namespace_mappings, **kwargs):
...
def setup_networks(networks_dir, network_settings, namespace_mappings, **kwargs):
    ...
    for network in network_settings:
        ...
        if not kwargs['force_download'] and os.path.isfile(network_file):
            print("%s already exists. Use --force-download to overwrite and re-map to uniprot")
            continue
        # download the file 
        #try:
...
def gzip_file(f1, f2, remove_orig=True):
    ...
    with open(f1, 'rb') as f_in:
        with gzip.open(f2+'.gz', 'wb') as f_out:

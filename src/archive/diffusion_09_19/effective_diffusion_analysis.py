...
def setup_opts():
    ...
    group.add_argument('--config', type=str, required=True,default = "fss_inputs/config_files/provenance/provenance_eval.yaml" ,
...
def main(config_map, **kwargs):
    ...
    *kwargs*: all of the options passed into the script
    ...
    # print('alg settings',alg_settings)
    # return
    ...
    for dataset in input_settings['datasets']:
        ...
        for alg_name in alg_settings:
            if alg_settings[alg_name]['should_run'][0]==True:
                ...
                #Nure: This statement perviously considered only genemaniaplus. now considers all algo with alpha as a param
                ...
                for alpha, alg in zip(alphas, alg_pred_files):
                    ...
                    #Nure: add alg_name to diff_mat_file, add exp_name

                    diff_mat_file = "%sdiffusion-mat-%s-%s-a%s.npy" % (net_obj.out_pref,alg_name,dataset['exp_name'],
                                                                       str(alpha).replace('.','_'))
                    ...
                    out_pref = "outputs/viz/%s/%s/diffusion-analysis/%s/cutoff%s-k%s-a%s%s" % (
                    ...
                    elif alg_name == 'sinksource':
                        M_inv = ss.get_diffusion_matrix(net_obj.W, pos_nodes_idx, alpha=alpha, diff_mat_file=diff_mat_file)
                    elif alg_name == 'sinksourceplus':
                        ss_lambda = alg_settings['sinksourceplus']['lambda'][0]
                        M_inv = ss.get_diffusion_matrix(net_obj.W, pos_nodes_idx, alpha=alpha, ss_lambda = ss_lambda,
                    ...
                    if alg_name=='genemaniaplus':
                        ...
                        beta_nodes_pos_nbr_dfsn[round(float(1/(1+alpha)),2)] = nodes_pos_nbr_dfsn
                    alpha_dfs[alpha] = df

                    sub_k = kwargs.get('sub_k')
                    out_file_path = "outputs/viz/%s/%s/diffusion-analysis/%s/per_source_node_contr/subk%s-a-%s%s.pdf" % (
                        dataset['net_version'], dataset['exp_name'], alg_name, sub_k, alpha, sig_str)
                    os.makedirs(os.path.dirname(out_file_path), exist_ok=True)
                    # if not os.path.exists(out_file_path):
                    plot_contribution_from_individual_sources(top_k_pred_idx[0:sub_k], pred_scores, pos_nodes_idx, M_inv, out_file_path)


                #out_pref = "outputs/viz/%s/%s/diffusion-comp/%s-%s-rand-set-diffusion-comp.pdf" % (
                out_pref = "outputs/viz/%s/%s/diffusion-analysis/%s/cutoff%s-k%s%s" % (
                    dataset['net_version'], dataset['exp_name'],alg_name, cutoff, k, sig_str)
                ...
                out_pref = "outputs/viz/%s/%s/diffusion-analysis/%s/k%s%s" % (
                    dataset['net_version'], dataset['exp_name'],alg_name, k, sig_str)
                ...
                if alg_name == 'genemaniaplus':
                    ...
                    plot_effective_diffusion(df_with_beta, out_file, xlabel="Beta", ylabel=ylabel, title="")

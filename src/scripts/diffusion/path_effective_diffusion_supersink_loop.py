...
def compute_and_save_contr_from_different_types_of_paths(M_pathmtx, R, pos_nodes_idx,
    ...
    frac_contr_from_st_path = {}
    frac_contr_from_sut_path = {}
    frac_contr_from_stst_path = {}
    frac_contr_from_stut_path = {}
    frac_contr_from_sust_path = {}
    frac_contr_from_suvt_path = {}
    frac_contr_from_path_len_beyond_3 = {}
    sut_contr_matrix = np.matmul(M,M) #M^2
    stut_contr_matrix = np.matmul(M, diags(sut_contr_matrix.diagonal()).A)
    sust_contr_matrix = np.matmul(diags(sut_contr_matrix.diagonal()).A, M)
    suvt_contr_matrix = np.matmul(M,sut_contr_matrix) #M^3
    ...
    for target in top_direct_targets:
        ...
        for source in pos_nodes_idx:
            st_path_cost += M[source][target]
            sut_path_cost += sut_contr_matrix[source][target]
            ...
            suvt_path_cost += suvt_contr_matrix[source][target] - sust_contr_matrix[source][target] \
                              - stut_contr_matrix[source][target] + stst_path_cost_indv

        frac_contr_from_st_path[target] = st_path_cost/total_incoming_contr
        frac_contr_from_sut_path[target] = sut_path_cost/total_incoming_contr
        ...
        frac_contr_from_path_len_beyond_3[target] = 1-(frac_contr_from_st_path[target]+\
            frac_contr_from_sut_path[target]+frac_contr_from_stst_path[target]+\
            frac_contr_from_stut_path[target]+frac_contr_from_sust_path[target]+ \
            frac_contr_from_suvt_path[target])

    del stut_contr_matrix, suvt_contr_matrix, sust_contr_matrix, sut_contr_matrix

    frac_contr_from_different_paths_df = pd.DataFrame({'target':list(frac_contr_from_stst_path.keys()),
                        'frac_st':list(frac_contr_from_st_path.values()),
                        'frac_sut':list(frac_contr_from_sut_path.values()),
                        ...
                        'frac_beyond_pathlen3':list(frac_contr_from_path_len_beyond_3.values()),
                        })

                ...
                if (alg_settings[alg_name]['should_run'][0] == True):
                    ...
                    for alpha, alg in zip(alphas, alg_pred_files):
                        ...
                        for ont in ['BP', 'MF', 'CC']:
                            ...
                            filtered_top_predictions = go_prep_utils.keep_prots_min_1_ann(top_preds,
                                                                                          go_category[ont], direct_prot_goids_by_c)
                            filtered_top_path_prots = go_prep_utils.keep_prots_min_1_ann(top_path_prots,
                                                                                         go_category[ont], direct_prot_goids_by_c)
                            ...
                            enrich_df_preds,enrich_out_file_preds = enrichment.run_clusterProfiler_GO(filtered_prot_universe,
                            ...
                            print('#top paths prots in query: ', len(filtered_top_path_prots),
                                  '\n #prots in universe: ', len(filtered_prot_universe))
                            enrich_df_paths, enrich_out_file_paths = enrichment.run_clusterProfiler_GO(

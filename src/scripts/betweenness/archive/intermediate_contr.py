...
def setup_opts():
    ...
    group.add_argument('--config', type=str, default="/data/tasnina/Provenance-Tracing/SARS-CoV-2-network-analysis/"
                                                     "fss_inputs/config_files/provenance/yeast/provenance_string400v11.5_s1.yaml"
    ...
    group.add_argument('--balancing-alpha-only', action='store_true', default=False,
                       help="Ignore alpha from config file rather take the alpha value\
                            ...
                            network-term-alg")
    group.add_argument('--betweenness-only', action='store_true', default=True,
                       help="If true only compute betweenness score. Thus do not need access to top predictions"
                            "in turns prediction score files.")
    return parser
...
def find_top_m_contributing_sources_per_pred(m, top_pred_idx, pos_nodes_idx, M_inv, cutoff=0):
    ...
    src_as_top_contr = {x: 0 for x in
                        pos_nodes_idx}  # key=source, value = #of top preds for which this source was a top contributor
...
def compute_frac_intermediate_nodes_contr_given_pos(M, R, pos_nodes_idx):
    '''
    ...
    2) Matrix, R: in matrix R, R(i,j) index hold the total contribution from node j to node i.
    3) list, pos_nodes_index. Contains the index of positive nodes in the network

    Function: 1) This will compute the contribution going via each node in the network from
    all known source nodes to a target node. So, we will get each node's contribution as an intermediate node
    to each target node's score.
    2) This will also compute what is frac of contribution made by an intermediate node to the final
    score of a target.
...
def compute_betweenness_score(M, idx_to_prot):
    ...
    M1 = M.T
    X1 = np.sum(M1, axis=0).reshape(-1, 1)  # column vector
            ...
            for alg_name in alg_settings:
                if (alg_settings[alg_name]['should_run'][0] == True) or (alg_name in kwargs.get('run_algs')):
                    ...
                    for alpha, alg in zip(alphas, alg_pred_files):
                        ...
                        # **********************************************
                        # The following analysis is focused on top predictions using given
                        # source nodes.
                        # We wanted to find out for each top predictions which intermediate
                        # nodes made the top contribution.
                        intermediate_nodes_contr, frac_intermediate_nodes_contr = \
                            compute_frac_intermediate_nodes_contr_given_pos \
                                (M_pathmtx_loginv, R, pos_nodes_idx)
                        ...
                        top_c_intermediate_prots_for_top_k_dict = \
                            analyse_intermediate_nodes_for_top_k(frac_intermediate_nodes_contr, top_k_pred_idx, prots,
                                                                 c=10)

...
def create_sparse_net_file(
    ...
    if forcenet is False \
       ...
       and os.path.isfile(net_names_file):
        ...
        #TODO currenly works for only one netx graph saving. if ever use multiple network this will not work.
        netx_graphs=[]
        netx_graphs.append(nx.read_gpickle(netx_graphs_file))
    ...
    print('initial netx_graphs:', type(netx_graphs), '\n', type(netx_graphs[0]))
...
def create_sparse_ann_file(
    ...
    *returns*:
    ...
    #Nure: commenting out forced part to make sur ethat the code will always create .npz file.
    ...
    print("\twriting sparse annotations to %s" % (sparse_ann_file))
    # store all the data in the same file
    #Nure: Also commenting out the following as I am not gonna use the saved file.
    # ann_matrix_data = get_csr_components(ann_matrix)
    # np.savez_compressed(
    #     sparse_ann_file, ann_matrix_data, terms, prots)
    # Nure: For some reason for GO terms, reading from already existing net file is not working. So commenting out the following
    #and making sure that the code always create new .npz file.
    # else:

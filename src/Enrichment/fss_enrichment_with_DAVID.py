...
def jaccard_histogram(jaccard_df_dict_before,jaccard_df_dict_after,out_file_base):
    ...
    for protein_set_name in jaccard_df_dict_before:
        ...
        if len(jaccard_before.index)>0 and len(jaccard_after.index)>0:

            print(protein_set_name)
            print('JACCARD df:', jaccard_before.shape, jaccard_after.shape)
...
def main(config_map, **kwargs):
    ...
    *kwargs*: all of the options passed into the script
    ...
    annotation_list = ['HIV_INTERACTION_PUBMED_ID', 'HIV_INTERACTION', 'HIV_INTERACTION_CATEGORY', 'UCSC_TFBS', 'UP_TISSUE', 'GAD_DISEASE',
     'GOTERM_BP_FAT', 'GOTERM_CC_FAT', 'GOTERM_MF_FAT','KEGG_PATHWAY', 'REACTOME_PATHWAY']
    ...
    all_dfs_DAVID = {d: pd.DataFrame() for d in annotation_list}


    terms_to_keep_GO = {g: [] for g in ['BP', 'CC', 'MF']}
    pathways_to_keep_KEGG=[]
    pathways_to_keep_Reactome = []
    terms_to_keep_DAVID = {d: [] for d in annotation_list}
    ...
    for dataset in input_settings['datasets']:
        ...
        for alg, pred_file in alg_pred_files.items():
            ...
            for k in k_to_test:
                ...
                DAVID_df_dict = enrichment.run_clusterProfiler_DAVID(topk_predictions, out_dir, annotation_list, prot_universe=prot_universe, forced=kwargs.get('force_run'),**kwargs)
                # f=open(out_dir+'/prots.txt','w')
                # for ele in topk_predictions:
                #     f.write(ele+'\n')
                # f.close()
                #
                # f=open(out_dir+'/prots_universe.txt','w')
                # for ele in prot_universe:
                #     f.write(ele+'\n')
                # f.close()

                for ann in DAVID_df_dict:
                    # if kwargs.get('compare_krogan_terms'):
                    #     krogan_result_to_look_into = 'DAVID-'+ann
                    #     DAVID_df_dict[ann] = add_qval_ratio(DAVID_df_dict[ann],krogan_result_to_look_into,krogan_dir,alg)

                    terms_to_keep_DAVID[ann] = terms_to_keep_DAVID[ann] + list(DAVID_df_dict[ann][DAVID_df_dict[ann]['p.adjust']<=kwargs.get('fss_pval')]['ID'])
                    tuples = [(dataset_name, alg, col) for col in DAVID_df_dict[ann].columns]
                    index = pd.MultiIndex.from_tuples(tuples)
                    DAVID_df_dict[ann].columns = index
                    all_dfs_DAVID[ann] = pd.concat([all_dfs_DAVID[ann], DAVID_df_dict[ann]], axis=1)
    ...
    for ann, d_df in all_dfs_DAVID.items():
        all_dfs_DAVID[ann] = d_df[d_df.index.isin(terms_to_keep_DAVID[ann])]
    ...
    if kwargs.get('compare_krogan_terms'):
        ...
        for ann, d_df in all_dfs_DAVID.items():
            df = include_Krogan_enrichment_result(krogan_dir,ann,d_df)
            all_dfs_DAVID[ann] = pd.concat([all_dfs_DAVID[ann], df], axis=1)
    ...
    for ann, df in all_dfs_DAVID.items():
        if kwargs.get('file_per_alg'):
            df = df.swaplevel(0,1,axis=1)
            for alg, df_alg in df.groupby(level=0, axis=1):
                df_alg.dropna(how='all', inplace=True)
                ...
                #if kwargs.get('compare_krogan_terms') and :
                # print(df_alg.head())
                out_file = "%s%s-k%s-%s.csv" % (out_pref, alg, k_to_test[0], ann)
                write_combined_table(df_alg, out_file, dataset_level=1)
        else:
            out_file = "%sk%s-%s.csv" % (out_pref, k_to_test[0], ann)
            # super_combined_df = pd.concat([super_combined_df, df],axis=0)
            processed_df= write_combined_table(df, out_file,dataset_level=0)
            with pd.ExcelWriter(super_combined_file, mode ='a') as writer:
                # print('processed_df: ', processed_df.shape)
                processed_df.to_excel(writer, sheet_name = 'DAVID-'+ ann)

            out_file_base = "%s_k%s_DAVID-%s" % (greedy_simplified_files_dir+network,k_to_test[0],ann)
            print('DAVID SIMPLIFICATION:', processed_df.shape)
            simplify_enrichment_greedy_algo(processed_df.copy(),1,out_file_base)

            # out_file_1 = "%s_k%s-%s_simplified.csv" % (greedy_simplified_files_dir+network,k_to_test[0],ann)
            # out_file_2 = "%s_k%s-%s_simplified_Jaccard.csv" % (greedy_simplified_files_dir+network,k_to_test[0],ann)
